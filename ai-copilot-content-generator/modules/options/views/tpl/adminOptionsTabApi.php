...
<section class="wbw-body-options-api">
	...
	<div class="wbw-settings-form row">
		...
		<div class="wbw-settings-fields col-10">
			<img src="<?php echo esc_url(WAIC_IMG_PATH . '/info.png'); ?>" class="wbw-tooltip" title="<?php echo esc_attr(__('Connect your Gemini API key to enable our plugin to leverage Google\'s advanced AI capabilities directly on your website. This key allows access to state-of-the-art natural language processing, reasoning, and multimodal AI features, ensuring smarter and more dynamic interactions.', 'ai-copilot-content-generator') . '<br>' . __('Don\'t have a Gemini account?', 'ai-copilot-content-generator') . ' <a href="#" target="_blank">' . __('Explore this guide', 'ai-copilot-content-generator') . '</a> ' . __('to create an account and obtain your API key.', 'ai-copilot-content-generator')); ?>">
			<div class="wbw-settings-field">
				<?php
				WaicHtml::text('api[gemini_api_key]', array(
					'value' => WaicUtils::getArrayValue($options, 'gemini_api_key', ''),
					'attrs' => 'aria-hidden="true" autocomplete="off" class="waic-fake-password" placeholder="' . esc_attr__('Enter your Gemini Api', 'ai-copilot-content-generator') . '"',
				));
				?>
			</div>
		</div>
	...
	<div class="wbw-settings-form row">
		...
		<div class="wbw-settings-fields col-10">
			<img src="<?php echo esc_url(WAIC_IMG_PATH . '/info.png'); ?>" class="wbw-tooltip" title="<?php echo esc_attr(__('Connect your DeepSeek API key to activate AI-powered content generation and language understanding on your website. With DeepSeek, you gain access to a powerful open-source AI model optimized for high-performance NLP tasks, delivering fast and efficient AI-driven solutions.', 'ai-copilot-content-generator') . '<br>' . __('Don\'t have a DeepSeek account?', 'ai-copilot-content-generator') . ' <a href="#" target="_blank">' . __('Explore this guide', 'ai-copilot-content-generator') . '</a> ' . __('to create an account and obtain your API key.', 'ai-copilot-content-generator')); ?>">
			<div class="wbw-settings-field">
				<?php
				WaicHtml::text('api[deep_seek_api_key]', array(
					'value' => WaicUtils::getArrayValue($options, 'deep_seek_api_key', ''),
					'attrs' => 'aria-hidden="true" autocomplete="off" class="waic-fake-password" placeholder="' . esc_attr__('Enter your DeepSeek Api', 'ai-copilot-content-generator') . '"',
				));
				?>
			</div>
		</div>
		...
		<div class="wbw-settings-fields col-10">
			...
			<div class="wbw-settings-field<?php echo ( 'deep-seek' != $curEngine ? ' wbw-hidden' : '' ); ?>" data-parent-select="api[engine]" data-select-value="deep-seek">
				...
				WaicHtml::selectbox('api[deep_seek_model]', array(
					...
					'value' => $curDeepSeekModel,
					'attrs' => 'id="waicApiDeepSeekModel" data-tokens="' . htmlentities(WaicUtils::jsonEncode($tokens), ENT_COMPAT) . '"',
				));
				?>
			</div>
			<div class="wbw-settings-field<?php echo ( 'gemini' != $curEngine ? ' wbw-hidden' : '' ); ?>" data-parent-select="api[engine]" data-select-value="gemini">
				...
				WaicHtml::selectbox('api[gemini_model]', array(
					...
					'value' => $curGeminiModel,
					'attrs' => 'id="waicApiGeminiModel" data-tokens="' . htmlentities(WaicUtils::jsonEncode($tokens), ENT_COMPAT) . '"',
				));
				?>
			</div>
		</div>
...
<?php if (empty($notShow['vision'])) { ?>
	<div class="wbw-settings-form row wbw-mb-ver10<?php echo ( 'deep-seek' == $curEngine ? ' wbw-hidden' : '' ); ?>" data-parent-select="api[engine]" data-select-value="open-ai gemini">
		...
		<div class="wbw-settings-fields col-10">
			...
			<div class="wbw-settings-field">
				...
				WaicHtml::selectbox('api[image_engine]', array(
					'options' => array('open-ai' => 'Open AI', 'gemini' => 'Gemini'),
