...
$props = $this->props;
?>
<div class="wbw-settings-form row waic-field-block" data-block="categories">
	...
	<div class="wbw-settings-fields col-10 waic-editable">
		...
		<div class="wbw-settings-field">
			...
			if (empty($data['g'])) {
				...
				$categories = WaicFrame::_()->getModule('workspace')->getTaxonomyHierarchy($fields[$field]['taxonomy'], $args);
				WaicHtml::selectlist('categories', array(
					'options' => $categories,
					'value' => WaicUtils::getArrayValue($data, 'r', ''),
					'class' => 'no-chosen',
				));
			} else {
				WaicHtml::text('categories', array(
