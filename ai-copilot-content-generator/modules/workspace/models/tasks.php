...
class WaicTasksModel extends WaicModel {
	...
	public function saveTask( $feature, $id, $params = array() ) {
		...
		if (empty($title)) {
			$title = 'Scenario ' . gmdate('Y-m-d-h-i-s');
		}
	...
	public function getHistory( $params ) {
		...
		$data = empty($data) ? array() : $this->_prepareHistoryForTbl($data);
		return array(
			'data' => $data,
			'total' => $totalCount,
		);
	...
	public function _prepareHistoryForTbl( $tasks ) {
		$rows = array();
		...
		foreach ($tasks as $task) {
			...
			$rows[] = array(
				...
				( empty($features[$feature]) ? '???' : $features[$feature] ),
				$task['tokens'],
				$statuses[$task['status']],
				$task['created'],
				( $user ? $user->get('display_name') : $task['author'] ),
				'<a href="' . $url . '&show_settings=1"><i class="fa fa-gear"></i></a>',
			);
		}
		return $rows;
	}
