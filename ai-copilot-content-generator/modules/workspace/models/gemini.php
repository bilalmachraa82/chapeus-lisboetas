...
class WaicGeminiModel extends WaicModel implements WaicAIProviderInterface {
	...
	private function getApiImageUrl() {
		switch ($this->imageModel) {
			...
			default:
				return $this->apiUrl . '/models/imagen-3.0-generate-002:predict?key=' . $this->apiKey;
		}
	}
	...
	public function getText( $params, $stream = null ) {
		...
		return $this->sendRequest($this->getApiChatCompletionsUrl());
	}

	private function sendRequest( $url, $method = 'POST', $type = '' ) {
		...
		if ('image' === $type && isset($data->candidates[0]->content->parts[0]->inlineData->data)) {
		...
		return array('results' => $results, 'params' => array('model' => $this->model));
	}
	...
	public function getImage( $params ) {
		...
		return $this->sendRequest($this->getApiImageUrl(), 'POST', 'image');
	}
}
